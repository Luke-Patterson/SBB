...
class NN_Logic:
    ...
    def load_char_position_models(self, folder = 'C:/AnacondaProjects/sbb/prod/models/char_position/'):
        for n, row in self.char_df.iterrows():
            char = row['name']
            for j in range(1,8):
                file = [i for i in os.listdir(folder) if char in i and 'position'+str(j)
                    in i and '.p' in i][0]
                self.load_model(label='Position'+str(j)+'_'+char,filepath =
                    folder + file)
        # repeat for No char models
        for j in range(1,8):
            char = 'None'
            file = [i for i in os.listdir(folder) if char in i and 'position'+str(j)
                in i and '.p' in i][0]
    ...
    # obsolete deploy decision code based off of win rates and board position predictions
    # def deploy_chars_decision(self):
    #     start = datetime.now()
